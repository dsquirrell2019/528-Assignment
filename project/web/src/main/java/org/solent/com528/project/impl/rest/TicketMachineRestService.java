...
public class TicketMachineRestService {
    ...
    public Response getTicketMachineConfig(@QueryParam("uuid") String uuid) {
        try {
            ...
            //MODIFY REST SERVICE TO GET TICKET MACHINE STATION AND ZONE
            
            //get ticket machine DAO
            TicketMachineDAO ticketMachineDAO = serviceFacade.getTicketMachineDAO();
            //Create ticket machine 
            TicketMachine machine = ticketMachineDAO.findByUuid(uuid);
            //set stationname and zone
            String stationName = machine.getStation().getName();
            Integer stationZone = machine.getStation().getZone();
